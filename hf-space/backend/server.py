...
# When the API and the frontend live on different domains (e.g. Vercel frontend
# calling a Hugging Face / Render backend), the browser will only store and send
# the auth cookies if they are marked Secure + SameSite=None. Both default to
# that here; set COOKIE_SECURE=false / COOKIE_SAMESITE=lax for local http dev.
...
# CORS
# The frontend sends requests with `withCredentials: true`. Browsers refuse to
# honour a credentialed response whose Access-Control-Allow-Origin is "*", so an
# explicit origin list is required in production. CORS_ORIGINS is a comma
# separated list, e.g. "https://investwise.vercel.app,http://localhost:5173".
_cors_origins = [o.strip() for o in os.environ.get('CORS_ORIGINS', '').split(',') if o.strip()]

if not _cors_origins or _cors_origins == ['*']:
    logger.warning(
        'CORS_ORIGINS is unset or "*". Falling back to wildcard with credentials '
        'DISABLED - cookie based login will not work from a browser. Set '
        'CORS_ORIGINS to your exact frontend URL(s) to enable it.'
    )
    app.add_middleware(
        CORSMiddleware,
...
async def startup_event():
    ...
    # NOTE: never log the admin password - Space/Render build logs are public.
    logger.info(f'Admin account ready: {ADMIN_EMAIL}')
